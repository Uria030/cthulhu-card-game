# 核心設計原則 補充 02：卡片效果語言與狀態系統 v0.1
## Core Design Principles Supplement 02: Card Effect Language & Status System v0.1
...
|------|------|------|------|
| 中毒 | `poison` | X | 受到的傷害 +X |
| 流血 | `bleed` | X | 回合結束時扣 X 點 HP |
| 燃燒 | `burning` | X | 回合開始時扣 X 點 HP。被火屬性攻擊增加 X 點傷害。移除潮濕 |
| 冷凍 | `frozen` | X | 普通移動花費 2 行動點。被冰冷攻擊增加 X 點傷害 |
| 黑暗 | `darkness` | — | 攻擊命中檢定 -2 |
| 繳械 | `disarm` | — | 本回合不能用資產進行攻擊 |
| 毀滅 | `doom_status` | X | 回合結束時受到 X 點傷害 |
| 疲勞 | `fatigue` | — | 結束階段不能抽牌跟獲得資源 |
| 發瘋 | `madness` | X | 受到的恐懼 +X |
| 標記 | `marked` | X | 受到的傷害和恐懼都 +X |
| 脆弱 | `vulnerable` | X | 受到的物理傷害 +X |
| 沈默 | `silence` | — | 無法施法 |
| 無力 | `weakness_status` | X | 近戰傷害降低 X 點 |
| 潮濕 | `wet` | X | 被電擊屬性攻擊時傷害 +X |
| 弱化 | `weakened` | — | 下次擲骰時丟兩次取差的，使用後移除 |

### 10.2 正面狀態（Buff）

| 狀態 | 代碼 | 參數 | 效果 |
|------|------|------|------|
| 強化 | `empowered` | — | 下次擲骰時丟兩次取好的，使用後移除 |
| 護甲 | `armor` | X | 降低 X 點物理傷害 |
| 護盾 | `ward` | X | 降低 X 點恐懼傷害 |
| 隱蔽 | `stealth` | — | 不觸發藉機攻擊，移動後解除 |
| 加速 | `haste` | — | 獲得額外行動點 |
| 再生 | `regeneration` | — | 回合開始時恢復 1 點 HP |
...
```json
{
  ...
  "effects": [
    ...
    },
    {
      ...
      "description_zh": "所有受影響的敵人獲得燃燒 1 狀態。",
      "description_en": "All affected enemies gain Burning 1."
    },
    {
      "trigger": "on_play",
      "condition": null,
...
```json
{
  ...
  "effects": [
    ...
    },
    {
      ...
      "target": "self",
      "effect_code": "add_status",
      "params": { "status": "stealth" },
      "duration": "until_end_of_turn",
      "description_zh": "若為夜間，你獲得隱蔽直到回合結束。",
      "description_en": "If it is nighttime, gain Stealth until end of turn."
    }
  ]
}
```
...
| 中毒 | Poison | 負面狀態，受到的傷害增加 |
| 流血 | Bleed | 負面狀態，回合結束時扣 HP |
| 燃燒 | Burning | 負面狀態，回合開始時扣 HP，火屬性攻擊增傷，移除潮濕 |
| 冷凍 | Frozen | 負面狀態，移動花費增加，冰屬性攻擊增傷 |
| 繳械 | Disarm | 負面狀態，不能用資產進行攻擊 |
| 毀滅狀態 | Doom Status | 負面狀態，回合結束時受到傷害 |
| 疲勞 | Fatigue | 負面狀態，不能抽牌和獲得資源 |
| 發瘋 | Madness | 負面狀態，受到的恐懼增加 |
| 標記 | Marked | 負面狀態，受到的傷害和恐懼都增加 |
| 脆弱 | Vulnerable | 負面狀態，受到的物理傷害增加 |
| 沈默 | Silence | 負面狀態，無法施法 |
| 無力 | Weakness Status | 負面狀態，近戰傷害降低 |
| 潮濕 | Wet | 負面狀態，被電擊攻擊時傷害增加 |
| 弱化 | Weakened | 負面狀態，下次擲骰取差的 |
| 強化 | Empowered | 正面狀態，下次擲骰取好的 |
| 護甲 | Armor | 正面狀態，降低物理傷害 |
| 護盾 | Ward | 正面狀態，降低恐懼傷害 |
| 隱蔽 | Stealth | 正面狀態，不觸發藉機攻擊 |
| 加速 | Haste | 正面狀態，獲得額外行動點 |
| 再生 | Regeneration | 正面狀態，回合開始時恢復 HP |
