...
## 品質閘四層制(2026-07-02 三連敗改善,全生產線共用設計)

1. **自動改寫級**(gate 不退回,改寫+記 log):及→與(複合詞保護)/跟→和(動詞豁免)/該玩家→你/我方→你的/相鄰重複標籤 dedupe——沿用 s06 §7.1 低風險自動修正先例
2. **規則唯一解自動糾正**:武器名→combat_style 對照表直接糾正,不退回
3. **定向重試**:違規全屬文字欄 → 只重寫違規欄位(成本 ~1/5),結構級才整份重跑
4. **敗因自動分診**:三連敗 error 自帶分類(文字級/格式級/結構級/機制訊號詞/數值合約級)

| 日期 | 批次 | 結果 |
|---|---|---|
| 2026-07-02 | 全 64 格完成(16 授權批 + Codex 依手冊跑 44 + 收尾 4) | **64/64 入庫**;三連敗改善四層落地 |
