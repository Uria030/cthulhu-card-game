...
- **is_extra：額外卡**（放在額外牌組、cost=0、遊戲中不可透過抽牌/檢索取得、由卡片 effects 的 desc_zh 自身說明召喚條件，例「當場上某某條件達成時，從額外牌組召喚此卡」）
...
23. **額外卡判定（is_extra=true）**：使用者描述若含「額外牌組」「不能主動抽」「由某條件召喚」等關鍵字，設 is_extra=true；cost 必 0；至少一條 effect.desc_zh 必須明確說明召喚條件（例「當場上累積 3 個毀滅標記時，從額外牌組召喚此卡」）；與 is_permanent 互斥。`;
};
...
7. **額外卡（is_extra=true）**：至少一條 effect.desc_zh 必須描述「如何被從額外牌組召喚」（例：「當你抽到神啟時」「當議程推進至第 3 張時」「當敵人被擊敗且主軸卡在場」等明確觸發條件）；額外卡不走打出流程。`;
};
