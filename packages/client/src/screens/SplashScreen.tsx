 ...
 * 三條螺旋觸手 120° 對稱環繞中心同心圓,以褪色黃銅色刻在門板上。
...
export function SplashScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      openTimer = window.setTimeout(() => setDoorOpen(true), COMPLETE_HOLD_MS);
      navTimer = window.setTimeout(() => navigate('/lobby'), COMPLETE_HOLD_MS + DOOR_OPEN_MS);
    });
    return () => {
      cancelled = true;
  ...
  return (
    <div className={'splash-root' + (doorOpen ? ' door-open' : '')}>
      <div className="splash-lobby-behind" aria-hidden />
      <div className="splash-door-stage" aria-hidden>
        <div className="splash-door-panel door-left" />
        <div className="splash-door-panel door-right" />
      </div>
      {/* The Yellow Sign(刻在門板上的褪色黃銅浮雕) */}
