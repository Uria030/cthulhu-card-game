...
// ============================================
// Migration 028: 戰役主線/支線類型 + 可變章節數
//   - campaigns 加 campaign_type ('main'/'side') 與 chapter_count
//   - chapters 移除 chapter_number BETWEEN 1 AND 10 的 CHECK,改由應用層用 chapter_count 把關
//
// 拆步驟以避免單一 DO 區塊內任何錯誤導致整段 rollback:
//   1. ADD COLUMN IF NOT EXISTS(無 CHECK)— 跨 PG 11+ 通用,既冪等也快
//   2. ADD CONSTRAINT 包 DO 區塊處理 duplicate_object
//   3. DROP CONSTRAINT IF EXISTS 移除舊的 1-10 限制
// ============================================
export const MIGRATION_028_SQL = `
ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS campaign_type VARCHAR(8) NOT NULL DEFAULT 'main';
ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS chapter_count INTEGER NOT NULL DEFAULT 10;

DO $$ BEGIN
  ALTER TABLE campaigns ADD CONSTRAINT campaigns_campaign_type_check
    CHECK (campaign_type IN ('main','side'));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  ALTER TABLE campaigns ADD CONSTRAINT campaigns_chapter_count_check
    CHECK (chapter_count BETWEEN 1 AND 10);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

ALTER TABLE chapters DROP CONSTRAINT IF EXISTS chapters_chapter_number_check;

COMMENT ON COLUMN campaigns.campaign_type IS '戰役類型:main=主線(預設 10 章)、side=支線(可 1-10 章)';
COMMENT ON COLUMN campaigns.chapter_count IS '此戰役實際章節數,主線通常 10、支線 1 起跳';
`;
...
// ============================================
// Migration 030: 補 M_013 失敗導致缺的 64 個 investigator_templates 骨架(用 8-attr 計算)
// 起因:M_019 改 faction_attribute_map (T→agility, P→reflex) + 加 attr_reflex DEFAULT 1 + chk 14-18(NOT VALID)
// 既有 deploy(M_013 在 M_019 之前已跑成功,row 已進)→ ON CONFLICT skip,本 M_030 無作用
// Railway 既有 deploy(已 M_019,但 INTP-1 等 row 不知何故消失)→ M_013 重跑 INSERT 觸發 chk(NOT VALID 對 INSERT 仍 enforce)→ 整批 rollback
// 修補:本 M_030 直接 INSERT 用 8-attr 算法(包含 attr_reflex),總和 = 14 滿足 chk
// 已驗算 16 個 MBTI × 4 career = 64 row,total 全 = 14 ✓
// ============================================
export const MIGRATION_030_SQL = `
WITH mbti_list AS (
  SELECT unnest(ARRAY[
    'INTJ','INTP','ENTJ','ENTP',
    'INFJ','INFP','ENFJ','ENFP',
...
SELECT
  ...
  1 + CASE WHEN main_attr_is('reflex',       s.mbti_code) THEN 3 ELSE 0 END + sub_attr_count('reflex',       s.mbti_code),
